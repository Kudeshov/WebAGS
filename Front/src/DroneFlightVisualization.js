  ...
  return (
    <Canvas camera={{ position: [-150, 100, -100], fov: 45 }}>  {/* [180, 0, 100]  [0, -180, 100]*/}
      ...
      {filteredMeasurements.map((measurement, index) => (
        <Point
          key={index}
          position={scaleCoordinates( measurement.lat, measurement.lon, measurement.alt, geoCenter.lat, geoCenter.lng)}
          color={getColor(measurement.dose, minDoseValue, maxDoseValue)}
        />
      ))}
      <OrbitControls />
    </Canvas>
